...
// import { DeliveryMethod, MonnifyTransaction, OrderItemStatus, OrderStatus, PaymentDetails, PaymentMethod, PaymentStatus, SettlementDetails } from './interfaces/order.interface';
...
//         // Validate size if product has variations
//         if (product.hasVariation && size) {
//           const validSizes = product.sizes.map(s => s.value);
...
//   /**
//    * Find orders for a specific business with pagination
//    * @param businessId - The business ID
//    * @param query - Pagination parameters
//    * @returns Promise<PaginationResultDto<Order>> - Paginated results
//    */
...
//       return new PaginationResultDto(data, {
//         itemCount,
//         pageOptionsDto: query || { skip, limit, order, page: Math.floor(skip / limit) + 1, },
//       });
//     } catch (error) {
//       if (error instanceof NotFoundException) {
//         throw error;
...
//         paymentMethods: this.getPaymentMethodsForMonnify(paymentMethod),
//         metaData: {
//           orderId: order.id,
...
//       // Call Monnify Initialize Transaction API
...
//   /**
//    * Get payment methods array for Monnify based on selected payment method
//    * @private
//    * @param paymentMethod - The selected payment method
//    * @returns string[] - Array of payment method strings for Monnify
//    */
//   private getPaymentMethodsForMonnify(paymentMethod: PaymentMethod): string[] {
...
//         ErrorHelper.BadRequestException(`Unsupported event type: ${eventType}`);
//       }
...
//           `${monnifyBaseUrl}/api/v1/merchant/transactions/query?paymentReference=${transactionReference}`,
//           {
//             headers: {
//               Authorization: `Bearer ${accessToken}`,
//               'Content-Type': 'application/json',
...
//       const transaction: MonnifyTransaction = verificationResponse.responseBody;
...
//           catchError((error: AxiosError) => {
//             this.logger.error(
//               `Monnify authentication failed: ${JSON.stringify(error.response?.data)}`,
//             );
//             ErrorHelper.InternalServerErrorException(
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (product.hasVariation && size) {
          ...
          const validSizes: string[] = product.sizes.flatMap(s => s.value);
