...
import { CreateAdminDto, UpdateAdminRoleDto, UpdateAdminStatusDto, AdminUserResponseDto } from './dto/admin-management.dto';
...
export class UsersService {
  ...
  async updateAdminRole(userId: number, newRole: UserRole): Promise<User> {
    const user = await this.userRepository.findOne({ where: { id: userId } });
    if (!user) {
      ErrorHelper.NotFoundException('Admin user not found');
    }
    user.role = newRole;
    return await this.userRepository.save(user);
  }

  async updateAdminStatus(userId: number, isActive: boolean): Promise<User> {
    const user = await this.userRepository.findOne({ where: { id: userId, role: In([UserRole.ADMIN, UserRole.SUPER_ADMIN]) } });
    if (!user) {
      ErrorHelper.NotFoundException('Admin user not found');
    }
    user.isActive = isActive; // Assuming your User entity has an 'isActive' field
    return await this.userRepository.save(user);
  }
  ...
  async getAdmins(): Promise<AdminUserResponseDto[]> {
    ...
    return admins as AdminUserResponseDto[];
  }

  async getAdminById(userId: number): Promise<AdminUserResponseDto> {
    const user = await this.userRepository.findOne({
      where: { id: userId, role: In([UserRole.ADMIN, UserRole.SUPER_ADMIN]) },
      select: ['id', 'firstName', 'lastName', 'email', 'phone', 'role', 'isActive', 'createdAt'],
    });
    if (!user) {
      ErrorHelper.NotFoundException('Admin user not found');
  ...
  async deleteAdmin(userId: number): Promise<void> {
    const result = await this.userRepository.delete({ id: userId, role: In([UserRole.ADMIN, UserRole.SUPER_ADMIN]) });
