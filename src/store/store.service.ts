...
export class StoreService {
  ...
  async createReview(reviewData: CreateReviewDto): Promise<Review> {
    ...
    if (!productInOrder) {
      throw new BadRequestException('Product is not part of the specified order');
    }
    ...
    if (existingReview) {
      throw new BadRequestException('Review already exists for this order. Only one review per order is allowed.');
    }
