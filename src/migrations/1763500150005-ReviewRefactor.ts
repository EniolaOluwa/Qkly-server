...
export class ReviewRefactor1763500150005 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `COMMENT ON COLUMN "reviews"."guestName" IS 'Guest reviewer name (when userId is null)'`,
    );
    await queryRunner.query(`ALTER TABLE "reviews" ADD "guestEmail" character varying`);
    await queryRunner.query(
      `COMMENT ON COLUMN "reviews"."guestEmail" IS 'Guest reviewer email (when userId is null)'`,
    );
    await queryRunner.query(`ALTER TABLE "reviews" ADD "guestPhone" character varying`);
    await queryRunner.query(
      `COMMENT ON COLUMN "reviews"."guestPhone" IS 'Guest reviewer phone (when userId is null)'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_058c6448644b27a8e63fd03106" ON "reviews" ("guestEmail") `,
    );
    await queryRunner.query(
