...
export class PaymentServiceAdapter1763285431205 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_85d65d236f62b4496cd115f097" ON "transactions" ("userId", "businessId") `,
    );
    await queryRunner.query(
      `ALTER TABLE "businesses" ADD "paystackSubaccountCode" character varying`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "businesses"."paystackSubaccountCode" IS 'Paystack subaccount code for split payments'`,
    );
    await queryRunner.query(
      `ALTER TABLE "businesses" ADD "revenueSharePercentage" numeric(5,2) NOT NULL DEFAULT '95'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "businesses"."revenueSharePercentage" IS 'Percentage business receives from sales (100 - platform fee)'`,
    );
    await queryRunner.query(
      `ALTER TABLE "businesses" ADD "isSubaccountActive" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "businesses"."isSubaccountActive" IS 'Whether subaccount is verified and active'`,
    );
    await queryRunner.query(`ALTER TABLE "businesses" ADD "settlementSchedule" character varying`);
    await queryRunner.query(
      `COMMENT ON COLUMN "businesses"."settlementSchedule" IS 'Settlement schedule for this business'`,
    );
    await queryRunner.query(`ALTER TABLE "users" ADD "paystackCustomerCode" character varying`);
    await queryRunner.query(
      `COMMENT ON COLUMN "users"."paystackCustomerCode" IS 'Paystack customer code'`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ADD "paystackDedicatedAccountId" character varying`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "users"."paystackDedicatedAccountId" IS 'Paystack dedicated account ID'`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ADD "paystackAccountStatus" character varying NOT NULL DEFAULT 'PENDING'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "users"."paystackAccountStatus" IS 'DVA account status'`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ADD "paymentProvider" character varying NOT NULL DEFAULT 'PAYSTACK'`,
    );
    await queryRunner.query(
      `COMMENT ON COLUMN "users"."paymentProvider" IS 'Payment provider for this user'`,
    );
    await queryRunner.query(`ALTER TABLE "orders" ADD "isRefunded" boolean NOT NULL DEFAULT false`);
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "refundedAmount" numeric(10,2) NOT NULL DEFAULT '0'`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "transactions" ADD CONSTRAINT "FK_9f24c9c8611e077a48a0645e3df" FOREIGN KEY ("businessId") REFERENCES "businesses"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "transactions" DROP CONSTRAINT "FK_9f24c9c8611e077a48a0645e3df"`,
    );
    await queryRunner.query(
